...
Manage 26 worker terminals inside the `turtlewolfe` tmux session.
...
tmux send-keys -t turtlewolfe:RoleName "command" Enter
```
...
# Attach/Detach
tmux attach -t turtlewolfe   # Ctrl+b d to detach
```
...
tmux send-keys -t turtlewolfe:WireframeGenerator1 "/clear" Enter
sleep 2
tmux send-keys -t turtlewolfe:WireframeGenerator1 "/prime wireframe-generator" Enter
```
