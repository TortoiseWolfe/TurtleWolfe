...
Manage the `stw` tmux session while coordinating with the main TurtleWolfe
...
17 wireframes exist in SpokeToWork-MVP but need quality review against TurtleWolfe standards.
...
| `stw/.terminal-status.json`                    | StW queue (separate from TurtleWolfe) |

## Authority

| Can Do                        | Cannot Do                      |
| ----------------------------- | ------------------------------ |
| Create `stw` tmux session     | Modify TurtleWolfe terminals   |
...
```
Main Operator (turtlewolfe session)
       │
...
The Main Operator focuses on TurtleWolfe internal work.
