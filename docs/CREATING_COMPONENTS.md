...
This guide explains how to create new components in TurtleWolfe following our enforced component structure standards.
...
docker compose exec turtlewolfe pnpm run audit:components

# Validate for CI/CD (exits with error if non-compliant)
docker compose exec turtlewolfe pnpm run validate:structure
```
...
docker compose exec turtlewolfe pnpm run migrate:components
```
...
```bash
docker compose exec turtlewolfe pnpm run audit:components -- --verbose
```
