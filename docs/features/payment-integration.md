...
1. Click "New project"
2. Fill in details:
   - **Name**: `turtlewolfe-payments` (or your project name)
...
Edit `/home/turtle_wolfe/repos/TurtleWolfe/.env` and add:
...
```bash
docker compose exec turtlewolfe supabase --version
...
docker compose exec turtlewolfe supabase link --project-ref $SUPABASE_PROJECT_REF --password $SUPABASE_DB_PASSWORD
```
...
```bash
# Push migrations
docker compose exec turtlewolfe supabase db push
...
```bash
# Generate types file
docker compose exec turtlewolfe sh -c 'supabase gen types typescript --linked > /app/src/lib/supabase/types.ts'

# Verify file created
docker compose exec turtlewolfe ls -lh /app/src/lib/supabase/types.ts
```
...
1. Click **Create App**
2. App Name: `TurtleWolfe Payments`
