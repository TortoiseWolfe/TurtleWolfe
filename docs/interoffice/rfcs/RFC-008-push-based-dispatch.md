...
2. Use `tmux send-keys -t turtlewolfe:N 'command' Enter` to push commands
...
```json
{
  "generator-1": "turtlewolfe:7",
  "generator-2": "turtlewolfe:8",
  "generator-3": "turtlewolfe:9",
  "planner": "turtlewolfe:6",
  "validator": "turtlewolfe:13"
}
```
...
```bash
tmux capture-pane -t turtlewolfe:7 -p | tail -1 | grep -q "bypass permissions"
```
