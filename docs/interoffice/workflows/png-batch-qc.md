...
tmux send-keys -t turtlewolfe:[terminal] Enter  # Select from autocomplete
sleep 3
...
```bash
tmux send-keys -t turtlewolfe:WireframeQA 'Review PNG batch: docs/design/wireframes/png/overviews_XXX/ - View each annotated PNG and log visual issues to .issues.md files.' Enter
...
```bash
tmux send-keys -t turtlewolfe:[terminal] '/clear' Enter
tmux send-keys -t turtlewolfe:[terminal] Enter  # Extra Enter for menu
```
...
```bash
tmux list-windows -t turtlewolfe | grep [terminal]
```
