...
**Status**: 📥 Inbox
**Created**: 2025-10-03
**Author**: TurtleWolfe Development Team
...
### TurtleWolfe Architecture Constraint
...
```bash
docker compose exec turtlewolfe pnpm add @supabase/supabase-js
docker compose exec turtlewolfe pnpm add -D supabase
```

**1.3 Initialize Supabase**

```bash
docker compose exec turtlewolfe npx supabase init
docker compose exec turtlewolfe npx supabase link --project-ref YOUR_PROJECT_REF
```
...
docker compose exec turtlewolfe npx supabase db push
```
...
docker compose exec turtlewolfe npx supabase functions deploy stripe-create-payment
docker compose exec turtlewolfe npx supabase functions deploy stripe-webhook
docker compose exec turtlewolfe npx supabase functions deploy paypal-create-order
docker compose exec turtlewolfe npx supabase functions deploy paypal-webhook
docker compose exec turtlewolfe npx supabase functions deploy payment-status
```
...
docker compose exec turtlewolfe pnpm run generate:component --name DonateButton --category atomic --hasProps true
docker compose exec turtlewolfe pnpm run generate:component --name CheckoutButton --category atomic --hasProps true
docker compose exec turtlewolfe pnpm run generate:component --name SubscribeButton --category atomic --hasProps true
docker compose exec turtlewolfe pnpm run generate:component --name PaymentConsentModal --category molecular --hasProps true
```
...
docker compose exec turtlewolfe npx supabase start

# Test Edge Functions locally
docker compose exec turtlewolfe npx supabase functions serve stripe-webhook

# Unit tests
docker compose exec turtlewolfe pnpm test src/utils/payment
docker compose exec turtlewolfe pnpm test src/components/payment
