...
docker compose exec turtlewolfe pnpm exec playwright test e2e/screenshot-current-state.spec.ts
```
...
docker compose exec turtlewolfe pnpm exec playwright test e2e/tests/blog-mobile-ux.spec.ts --project="iPhone 12"
...
docker compose logs turtlewolfe | grep -i error
```
...
docker compose exec turtlewolfe pnpm exec playwright test e2e/screenshot-after-quickfix.spec.ts
...
- [ ] Run `docker compose exec turtlewolfe pnpm run test:a11y:dev`
...
docker compose exec turtlewolfe pnpm list tailwindcss
...
docker compose exec turtlewolfe pnpm exec playwright install
...
docker compose exec turtlewolfe npx lighthouse http://localhost:3000 --preset=mobile --quiet
