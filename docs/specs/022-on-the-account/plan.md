...
**Constitution File**: `.specify/memory/constitution.md` (TurtleWolfe Constitution v1.0.1 - 6 core principles)
...
# Verify
docker compose exec turtlewolfe npx supabase db execute \
...
```bash
docker compose exec turtlewolfe pnpm add react-easy-crop
```
...
```bash
# Unit tests
docker compose exec turtlewolfe pnpm test src/lib/avatar
docker compose exec turtlewolfe pnpm test src/components/atomic/AvatarUpload

# E2E tests
docker compose exec turtlewolfe pnpm run test:e2e
