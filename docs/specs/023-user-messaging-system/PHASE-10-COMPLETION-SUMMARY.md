...
**Status**: COMPLETE
**Location**: `/home/turtle_wolfe/repos/TurtleWolfe/CLAUDE.md`
...
```bash
docker compose exec turtlewolfe pnpm add @radix-ui/react-dialog
```
...
2. **Run Production Build Test** (5 min)
   ...
   ```bash
   docker compose exec turtlewolfe pnpm run build
   docker compose exec turtlewolfe ls -la out/
   ```
