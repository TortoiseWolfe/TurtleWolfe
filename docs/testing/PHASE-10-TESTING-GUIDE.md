...
docker compose exec turtlewolfe pnpm exec pa11y http://localhost:3000/messages --runner axe
docker compose exec turtlewolfe pnpm exec pa11y http://localhost:3000/messages/connections --runner axe
docker compose exec turtlewolfe pnpm exec pa11y http://localhost:3000/conversations --runner axe
```
...
docker compose exec turtlewolfe pnpm exec serve out -p 3000

# Terminal 3: Run Lighthouse
docker compose exec turtlewolfe pnpm exec lighthouse http://localhost:3000/messages \
...
# Analyze bundle
docker compose exec turtlewolfe pnpm exec next build --experimental-build-mode=compile
docker compose exec turtlewolfe ls -lh .next/static/chunks
```
...
2. Re-run build: `docker compose exec turtlewolfe pnpm run build`
