...
docker compose exec turtlewolfe pnpm run type-check    # TypeScript
docker compose exec turtlewolfe pnpm run lint           # ESLint
docker compose exec turtlewolfe pnpm run format:check   # Prettier
docker compose exec turtlewolfe pnpm test              # Unit tests
docker compose exec turtlewolfe pnpm run test:coverage # Coverage
docker compose exec turtlewolfe pnpm run test:a11y     # Accessibility
```
...
```bash
docker compose exec turtlewolfe pnpm run e2e:accessibility
```
...
docker compose exec turtlewolfe node scripts/validate-structure.js
```
