---
title: 'Building a Centralized Admin Dashboard for TurtleWolfe'
...
ogTitle: Building a Centralized Admin Dashboard for TurtleWolfe
...
TurtleWolfe deploys to GitHub Pages as a static export. That constraint shapes every architectural decision:
...
The messaging domain deserves special attention. TurtleWolfe uses End-to-End (E2E) encryption for messages — the `encrypted_content` and `iv` (initialization vector) columns on the `messages` table contain ciphertext that only the sender and recipient can decrypt.
...
**Want to explore the full implementation?** Check out the [TurtleWolfe GitHub repository](https://github.com/TortoiseWolfe/TurtleWolfe). The admin service classes live in `src/services/admin/`, the page routes in `src/app/admin/`, and the RLS policies in `supabase/migrations/20251006_complete_monolithic_setup.sql`.
