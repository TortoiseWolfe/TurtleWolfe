...
```
origin  git@github.com:TortoiseWolfe/TurtleWolfe.git
```
...
```bash
git remote set-url origin git@github.com:TortoiseWolfe/TurtleWolfe.git
```
...
tmux capture-pane -t turtlewolfe:18 -p | tail -20

# Check if stuck on permission prompt
tmux capture-pane -t turtlewolfe:$i -p | grep -q "Do you want to proceed"
```
