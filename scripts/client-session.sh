...
# Color palette - match TurtleWolfe but slightly different for distinction
...
COLOR_BASE="colour239"       # Darker gray (distinguishes from turtlewolfe)
...
case "$CLIENT" in
  stw)
    # SpokeToWork repos are siblings to TurtleWolfe
    CLIENT_DIR="$(dirname "$PROJECT_DIR")/SpokeToWork-MVP"
    if [ ! -d "$CLIENT_DIR" ]; then
      echo "Warning: $CLIENT_DIR not found, using TurtleWolfe dir"
...
# Status bar styling (slightly different from turtlewolfe for visual distinction)
