...
```bash
docker compose exec turtlewolfe pnpm test src/lib/auth/oauth-utils.test.ts
```
...
```bash
# Unit tests
docker compose exec turtlewolfe pnpm test oauth-utils

# E2E test (if available)
docker compose exec turtlewolfe pnpm exec playwright test welcome-message
```
